...
function paintBoard(kind){
  ...
  function render(rows){
    ...
    rows.forEach(function(r,i){
      ...
      var val = kind==='coin' ? ('🪙 '+(r.coins||0)) : kind==='game' ? ('🎮 '+(r.gameScore||0)) : kind==='time' ? ('⏱️ '+fmtTime(r.gameTime) + (r.gameDiff ? ' (' + ['Easy','Normal','Hard','VHard','Hell','Imp'][r.gameDiff] + ')' : '')) : ('Lv '+l);
...
function showUser(uid){
  ...
  body.innerHTML = '<button class="sc-back" id="ppComBack">'+PPI18n.t('community.back')+'</button>'
    ...
    + '<div class="statpill"><b>⏱️ '+fmtTime(r.gameTime||0)+'</b><span>'+PPI18n.t('community.fastestclear')+(r.gameDiff ? ' ('+['Easy','Normal','Hard','VHard','Hell','Imp'][r.gameDiff]+')' : '')+'</span></div></div>'
