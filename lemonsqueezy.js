...
/* --------------------------------------------------------------- public */
/* Check if user is logged in (before purchase) */
function requireLogin(){
  if(typeof currentUser !== 'undefined' && currentUser) return true;
  /* Show a friendly reminder */
  if(typeof PPI18n !== 'undefined'){
    if(typeof toast === 'function') toast(PPI18n.t('purchase.login.required','Please sign in or create an account first — Pro is linked to your account.'),'info');
    else alert(PPI18n.t('purchase.login.required','Please sign in or create an account first — Pro is linked to your account.'));
  } else {
    alert('Please sign in or create an account first — Pro is linked to your account.');
  }
  /* Open auth modal */
...
window.PPLemon = {
  ...
  buyPro:   function(){ if(!requireLogin()) return; openUrl(BUY_PRO); },
